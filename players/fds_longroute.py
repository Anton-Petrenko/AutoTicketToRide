...
class LongRoute(Player):

    def __init__(self, name = "LongRouteAgent"):
        super().__init__(name)
    ...
    def decide(self, game: GameEngine):
        ...
        for move in possible_moves:
            if move.type == PLACE_ROUTE: claim_route_moves.append(move)
            elif move.type == DRAW_FACEUP or move.type == DRAW_FACEDOWN: draw_train_card_moves.append(move)
    ...
    def generate_game_plan(self, game: GameEngine):
        joint_graph = self.joint_graph(game, 3 if game.options.players[game.player_making_move].trains_left >= 3 else game.options.players[game.player_making_move].trains_left)
        city1 = None
        city2 = None
        color = None
    ...
    def chooseNextRouteTarget(self, game: GameEngine, graph: nx.MultiGraph, city1, city2):
        try:
    ...
    def chooseDestinations(self, moves: list[Action], game: GameEngine, min_weight_edge: int):
        best_move = (0, None)
        ...
        for m in moves:
            ...
            for dest in m.destinations:
                temp = self.calculate_value(dest, joint_graph)
    ...
    def calculate_value(self, dest: DestinationCard, graph: nx.MultiGraph):
        try:
            if nx.has_path(graph, dest.city1, dest.city2):
                left_to_claim = 0
                path = nx.shortest_path(graph, dest.city1, dest.city2)
                ...
                return [float(dest.points), float(left_to_claim)]
            else:
                return [float(-1.0 * dest.points), float(50)]
        except:
            return [float(-1.0 * dest.points), float(50)]

    def joint_graph(self, game: GameEngine, min_weight_edge=0):
        free_connections_graph = self.free_routes_graph(game, min_weight_edge)
    ...
    def free_routes_graph(self, game: GameEngine, min_weight_edge: int = 0):
        graph = game.board
